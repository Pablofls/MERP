...
const DEFAULTS = ["Personal", "Morfosys", "Kenet"];

type CategoriaDB = { id: string; nombre: string; user_id: string; created_at: string };
    ...
    supabase
      ...
      .then(async ({ data }) => {
        ...
        if (data.length > 0) {
          ...
          const inserts = DEFAULTS.map((nombre) => ({ nombre, user_id: user.id }));
